...
**Aの理由：** プロジェクトスキルは同じ名前の個人スキルよりも優先されます。チームのスキルと一緒に個人化されたバリアントを利用できるようにするには、開発者は個人の`~/.claude/skills/`ディレクトリに異なる名前のスキル（例：`/my-commit`）を作成すべきです。
